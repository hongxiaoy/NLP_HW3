...
`max_matching` 目录下是基于匹配的分词方法代码，`bilstm_crf` 目录下是基于神经网络的分词方法代码。
...
```bash
cd bilstm_crf/data
python create_data.py  #构建数据
cd ..
python train.py  #训练模型
python inference.py  #推理新的句子
```
...
在基于神经网络的分词方法中，我使用了 LSTM + CRF 模型。评价模型性能的方式就是记录模型的预测输出与真值标签之间的重合情况，求得预测的准确率、召回率等指标。
...
代码位于 `max_matching` 目录下的 `bpe.py` 文件中。
...
'现代化建设', '解放思想实事求是', '扶贫工作', 
'先进典型', '全国各族', '有中国', '中国特色', 
'经济建设', '归侨侨眷', '世界各国', '特色社会主义', 
'伟大旗帜', '十五大精神', '基本路线', '新年音乐会', 
'贫困地区', '国家主席', '广播电台', '全国代表大会', 
'高举邓小平理论', '贯彻落实', '团结一致', 
'劳动模范代表', '中共中央政治局', ...]
```
...
对于中文分词，由于中文语言的复杂性，传统的基于规则或统计的分词方法可能无法完全准确地切分出所有的命名实体。子词压缩技术可以提供更加精细的分词粒度，将文本切分成更小的子词单元，从而有可能提高命名实体的识别精度。

例如，对于一个包含多个音译外来词或新词的句子，传统的分词方法可能无法准确地切分出这些词。子词压缩技术可以将这些音译外来词或新词切分成更小的子词单元，从而更好地捕捉其语义信息。

因此，子词压缩可以作为一种辅助技术，提高中文分词中命名实体的识别精度。但是，子词压缩并不能完全替代传统的中文分词方法，而是可以与其结合使用，以达到更好的分词效果。