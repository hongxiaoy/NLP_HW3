...
def main():
    ...
    for epoch in trange(total_epoch):
        ...
        preds_val = torch.vstack(preds_val)
        ...
        if precision > best_precision:
            torch.save(model.state_dict(), f'lstm_softmax_partition_best_at_epoch_{epoch+1}.pth')
            best_precision = precision
        
        writer.add_scalar('validation Precision', TP/TP_and_FP, epoch)
        writer.add_scalar('validation Recall', TP/TP_and_FN, epoch)
        
        print(f"\nEpoch (Val): {epoch+1}, Precision: {TP / TP_and_FP}, Recall: {TP / TP_and_FN}")
